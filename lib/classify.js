...
function buildPrompt(taxonomy) {
  const domainContext = taxonomy.promptContext
    ...
    : '';
  const knownEquipment = taxonomy.equipment?.length
    ? `\nKnown equipment seen in previous videos from this project: ${taxonomy.equipment.join(", ")}. Include these if you see them, and add any new ones you spot.`
    : '';
  const knownProducts = taxonomy.products?.length
    ? `\nKnown products/brands seen previously: ${taxonomy.products.join(", ")}. Include these if you see them, and add any new ones you recognise.`
    : '';
  const knownTechniques = taxonomy.techniques?.length
    ? `\nKnown techniques seen previously: ${taxonomy.techniques.join(", ")}. Include these if relevant, and add any new ones.`
    : '';
  const knownDescriptors = taxonomy.subjectDescriptors?.length
    ? `\nKnown subject descriptors: ${taxonomy.subjectDescriptors.join(", ")}. Include these if relevant, and add any new ones.`
    : '';
...
3. shotType — one of: ${taxonomy.shotTypes.join(", ")}
   ...
   - "cutaway": brief insert shot of a detail or reaction
   - "establishing": wide shot setting a scene or location
...
5. equipment — array of physical objects, tools, or devices visible in the shot. Use short lowercase slugs (e.g. "laptop", "dji-mic", "tripod", "whiteboard"). Be specific.${knownEquipment}
6. products — array of specific brand or product names you can identify (e.g. "DJI Mic 2", "Sony A7 IV", "Apple MacBook Pro"). Only include if you are confident in the identification.${knownProducts}
7. technique — array of actions or techniques being performed (e.g. "typing", "writing", "pointing-at-screen", "adjusting-camera"). Use lowercase slugs.${knownTechniques}
8. subjectDescriptors — array of descriptive tags about the subject matter or visual style (e.g. "indoor", "natural-light", "studio", "outdoor", "slow-motion"). Use lowercase slugs.${knownDescriptors}
...
11. other — array of notable visual elements (e.g. "text-overlay", "motion-blur", "reflection", "bokeh")
